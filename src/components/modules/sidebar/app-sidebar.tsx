...
const AppSidebar = () => {
    ...
    return (
        <Sidebar>
            <SidebarContent>
                <SidebarGroup>
                    <h1 className="text-2xl font-semibold border-b-2 border-green-500 pb-3 text-center"><span className="text-2xl text-green-500 ">Green</span> Circle</h1>
                    <SidebarGroupContent>
                        <SidebarMenu className="mt-10">
                            {items.map((item) => (
                                <SidebarMenuItem key={item.title}>
                                    <SidebarMenuButton asChild>
                                        <Link className={` ${pathname === item.url ? "bg-green-500 text-white" : ""} text-[16px] rounded-none `} href={item.url}>
                                            <item.icon className={`mr-2 ${pathname === item.url ? "text-white" : ""} text-green-500 h-5 w-5`} />
            ...
            <Avatar className="mb-3 mx-2">
                <div className="flex gap-2">
                    ...
                    <div>
                        ...
                        <p className="font-semibold text-[14px] text-green-500">{myProfile?.email}</p>
                    </div>
                    <Popover>
                        ...
                        <PopoverContent className="relative left-52">
                            <div>
                                ...
                                <ul className="divide-y">
                                    <li onClick={handleLogout} className="mt-4  flex gap-2 cursor-pointer hover:bg-green-500 p-1 hover:text-white"><LogOut className="relative top-1" size={18} /> Logout</li>
                                    <li className="mt-2  flex gap-2 cursor-pointer hover:bg-green-500 p-1 hover:text-white">< Bell className="relative top-1" size={18} /> Notification</li>
                                </ul>
                            </div>
                        </PopoverContent>
                    </Popover>

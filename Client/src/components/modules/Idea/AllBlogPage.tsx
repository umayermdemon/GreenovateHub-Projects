                    ...
                    {data?.map((blog) => (
                        <div key={blog.id} className="lg:flex justify-between pb-5">
                            <div className="flex items-center gap-3 lg:w-[40%]">
                                ...
                                <div>
                                    <p className="font-medium">{blog.title.split(" ").slice(0, 6).join(" ")}</p>
                                    <p className="text-sm text-muted-foreground">
                                        by <span className="italic text-amber-800">{blog.author.name}</span> |{" "}
                     ...
                     ${blog.status === "approved" ? "bg-green-100 text-green-500" : ""}
                     ${blog.status === "rejected" ? "bg-red-100 text-red-500" : ""}
                     ${blog.status === "underReview" ? "bg-yellow-300 text-amber-600" : ""}`}>
                                        <span
                        ...
                        ${blog.status === "underReview" ? "bg-amber-600" : ""}`}
                                    ...
                                    <p className="bg-amber-500 text-white px-2 py-1 rounded-full w-[120px] text-center truncate">{blog.category}</p>
                                </div>
                                <div className="flex lg:gap-8 justify-evenly lg:mt-0 mt-2">
                                    <Link href={`/admin/dashbord/all-blogs/details/${blog.id}`}>
                                    ...
                                    <Button onClick={() => deleteBlog(blog.id)} size="sm" className="bg-red-500 text-white h-8 w-8 p-0">

          ...
          {data?.map((idea: TIdea) => (
            <div key={idea.id} className="flex items-center justify-between ">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">
                    {idea.title.split(" ").slice(0, 5).join(" ")}
                  </p>
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => rejectRequest(idea.id)}
                  size="sm"
                  variant="outline"
                  className="h-8 w-8 p-0 cursor-pointer hover:bg-red-500 hover:text-white">
                  <X className="h-4 w-4" />
                  <span className="sr-only">Decline</span>
                </Button>
                <Button
                  onClick={() => approveRequest(idea.id)}
                  size="sm"
                  className="h-8 w-8 p-0 cursor-pointer bg-amber-500">
